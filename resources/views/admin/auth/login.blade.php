...
<x-layouts.app title="Connexion administrateur">
    <div class="mx-auto flex min-h-screen max-w-[380px] flex-col items-center justify-center gap-6 px-4">
        <div class="inline-flex items-center justify-center rounded-2xl bg-[linear-gradient(135deg,{{ $clubSetting?->secondary_color ?? '#17A8E5' }}_0%,{{ $clubSetting?->primary_color ?? '#0B73C5' }}_55%,#0A5CA6_100%)] p-3 shadow-[0_8px_20px_rgba(10,92,166,.35)]">
            <img src="{{ $clubSetting?->logoUrl() ?? asset('assets/ife-logo.png') }}" alt="{{ $clubSetting?->name ?? 'RC Cotonou Ife' }}" class="h-16 w-auto object-contain">
        </div>
